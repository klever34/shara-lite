...
export const BNPLRecordTransactionScreen = withModal(
  (props: BNPLRecordTransactionScreenProps) => {
    ...
    const bnplProducts = useMemo(
      () =>
        bnplBundles?.map((item) => {
          const amountToPay =
            (((item.interest_rate ?? 0) * (item.payment_frequency ?? 0)) /
              100) *
              credit_amount +
            credit_amount;

          const total_amount = values.bearingFees ? credit_amount : amountToPay;
          const merchant_amount =
            credit_amount /
            (((item.interest_rate ?? 0) * (item.payment_frequency ?? 0) + 100) /
              100);
          ...
          return {
            ...
            payment_frequency_amount:
              total_amount / (item.payment_frequency ?? 0),
          };
        }),
      [credit_amount, values.bearingFees, bnplBundles?.length],
    );

    const [selectedBNPLProduct, setSelectedBNPLProduct] = useState<
      BNPLBundle | undefined
    >();
      ...
      (values) => {
        openModal('bottom-half', {
          renderContent: () => (
            <ConfirmationModal
              onClose={closeModal}
              onSubmit={() => handleOpenPinModal(values)}
            />
          ),
        });
      },
    ...
    return (
      <SafeAreaView style={applyStyles('bg-white flex-1')}>
        ...
        {!!values.total_amount && !!bnplProducts?.length && (
          <ScrollView persistentScrollbar keyboardShouldPersistTaps="always">
            <View style={applyStyles('px-24')}>
              ...
              <View style={applyStyles('center pb-16')}>
                <Text
                  style={applyStyles('text-400 text-gray-100 text-uppercase')}>
              ...
              <Text
                style={applyStyles(
                  'pt-8 text-red-100 text-center text-uppercase',
                  {paddingBottom: 100},
                )}>
                {strings('bnpl.record_transaction.repayment_date', {
                  date: format(
                    addWeeks(
                      new Date(),
                      selectedBNPLProduct?.payment_frequency ?? 1,
                    ),
                    'dd MMM, yyyy',
                  ),
                })}
